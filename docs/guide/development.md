...
│   ├── config-schema.ts # snowLumaPlugin.configSchema（与 openclaw.plugin.json 里的 schema 保持同步）
│   ├── env.ts           # SNOWLUMA_* 环境变量回退（仅 default 账号）
│   ├── segments.ts      # 入站消息段解析/归一化（toSegments / renderSegments / sanitizeDisplayName）
│   ├── triggers.ts      # mention 模式判定逻辑（evaluateTrigger / matchKeyword / stripLeadingMention）
│   ├── aggregator.ts    # realtime / digest / history 三套独立的聚合引擎（createAggregator）
│   ├── summary.ts       # /summary 命令：matchSummaryCommand + runSummaryCommand（绕过聚合器）
│   ├── markdown-text.ts # 总结回复的 Markdown → 纯文本拍平（无运行时依赖）
...
│   ├── params.ts        # tools.ts 用的本地 readStringParam / readNumberParam（不依赖 openclaw）
│   ├── sdk.ts           # @snowluma/sdk 的自打补丁懒加载器（唯一按值导入 SDK 的地方）
│   ├── plugin-entry.ts  # 本地版 defineChannelPluginEntry，让 index.js 的图不含 openclaw 导入
...
└── test/
    ...
    └── *.test.ts                 # 20 个测试文件，419 个用例（核心 10 个见下表）
```
...
`npm test`（`vitest run`）目前运行 **20 个测试文件，419 个用例**，全部通过；下表列出其中最核心的 10 个文件；`npm run coverage` 额外要求 `src/**/*.ts`（`types.ts` 排除在外，纯类型声明没有可执行分支）达到行 80% / 函数 80% / 分支 70% / 语句 80% 的覆盖率门槛（`vitest.config.ts`）。
...
| `aggregator.test.ts` | 27 | realtime 引擎的开窗/并入/四种 flush 触发条件；digest 引擎的 scope/peers 过滤、`minMessages` 抑制-重试、`maxTranscriptChars` 裁剪；`flushAll`/`dispose`；`onFlush` 抛错时的隔离（一个批次的错误不影响另一个批次）。 |
| `segments.test.ts` | 26 | `toSegments` 处理数组/CQ 字符串/纯字符串三种入站消息形状及其退化路径；`extractText`/`extractMentions`（含 `atAll`）/`extractImageUrls`/`extractRecordUrls`/`extractReplyToId`/`extractForwardIds`；`renderSegments` 的占位符渲染与 @ 群名片清洗；`sanitizeDisplayName` 对昵称里换行/方括号的折叠；`normalizeMessageEvent` 端到端归一化。 |
| `outbound.test.ts` | 43 | `parseTarget` 的四种写法与非法输入拒绝；`formatTarget`；`chunkText` 的换行优先切分与 CQ 码不可分割保证；`sendText` 的分块与仅首块引用；`sendMedia` 的图片/语音/文件上传三路由与 caption 独立发送；`reactToMessage` 的成功/失败包装。 |
...
| `quote.test.ts` | 17 | `resolveQuoteContext` 的关闭开关与"无引用无转发直接返回 null"两条门禁；`get_msg` 成功解析路径；`resolveForwardNodes` 的深度/节点数/环检测展开逻辑；引用与转发整合后的字符预算截断；`formatQuoteContext` 的渲染格式（含无发送者信息、截断标记两种退化）与发送者昵称的清洗。 |
| `gateway.test.ts` | 30 | `createSelfMessageTracker` 的 FIFO 容量淘汰；`startGateway` 的建连/`selfId` 探测/`onReady`/`onError` 回调；消息路由（自身消息过滤、`isPeerAllowed`、触发判定接入聚合器、`groupAutoReact`）；自身消息 id 追踪如何反哺 `reply-to-self` 判定；`abortSignal` 触发后的优雅关闭顺序（flush → unregister → close）。 |
| `dispatch.test.ts` | 49 | `resolveInboundCommandAuthorization` 的访问组网关与 `allowFrom` 兜底；`buildBatchBody` 对 realtime/digest 两种批次的正文组装差异；`dispatchBatch` 的 realtime 端到端流程（含引用解析接入）；带历史时发言人标签落在当前消息上而非历史块前（含一条用宿主真实 `formatInboundEnvelope` 跑的回归）；昵称清洗后无法伪造出多余的转录行；digest 批次的 `CommandAuthorized` 硬编码为 `false`；`SKIP` 静默（含被 Markdown 装饰过的 SKIP）；`replyToTrigger` 引用哪条消息；媒体+文本混合投递；`dispatchBatch` 对各种子步骤异常"永不 reject"的保证。 |
| `tools.test.ts` | 22 | `createSnowLumaAgentTools` 的工具装配；`snowluma_get_history` 的参数校验/`count` 夹取/分页/排序/渲染/失败降级；`snowluma_get_group_members` 的 `groupId` 校验/`limit` 夹取/截断提示/渲染；两个工具结果里昵称/群名片的清洗（一条记录只占一行）；依赖注入（`ToolDeps.acquireActionClient` 替身）。 |
